...
const REMOTE_REDIRECT_URI = 'http://localhost/callback';
...
async function authCodeFlowLogin(clientId, clientSecret, scopes, noBrowser) {
  ...
  if (!code) {
    ...
    console.error(`Waiting for authorization callback on http://localhost:${port}${REDIRECT_PATH} ...`);
    code = await waitForCallback(server, state).promise;
  }
...
function generateRemoteAuthUrl(clientId, scopes) {
  const oauth2 = new OAuth2Client(clientId, null, REMOTE_REDIRECT_URI);
...
async function exchangeCode(callbackUrl) {
  ...
  const oauth2 = new OAuth2Client(clientId, clientSecret, REMOTE_REDIRECT_URI);
  const { tokens } = await oauth2.getToken(code);
...
async function login(extraScopes = [], { noBrowser = false, flow, remote = false } = {}) {
  const { clientId, clientSecret } = getCredentials();
  ...
  if (remote) {
    if (!useAuthCode && flow !== 'authcode') {
      // Remote + device flow: device flow works fine remotely
      const tokens = await deviceFlowLogin(clientId, clientSecret, scopes);
      tokenStore.save(tokens);
      console.error('Login successful. Token cached at ~/.config/gws-auth/tokens.json');
      return;
    }
    const authUrl = generateRemoteAuthUrl(clientId, scopes);
    console.error(`Open this URL in a browser:\n\n  ${authUrl}\n`);
    console.error(`After authorization, the browser will redirect to a URL like:`);
    console.error(`  http://localhost/callback?code=4/0AQ...\n`);
    console.error(`Copy the full URL and run:`);
    console.error(`  gws-auth exchange "<the-full-url>"`);
    return;
  }
